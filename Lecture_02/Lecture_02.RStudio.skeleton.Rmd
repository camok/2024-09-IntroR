...
### 0.4.4.1 Dataset 1: /data/infection_metadata.csv
...
::: {align="center"}
<img src="https://github.com/camok/CSB_Course_Materials/blob/main/IntroR/I_made_this_package.png?raw=true" width="700"/>
:::
...
::: {.alert .alert-block .alert-warning}
**Caution:** Note from our above example that we no longer have proper column headings! Rather the column names have been derived from the data existing in row **A3**. If you had a complex header or were subsetting by a range like this you would include some additional steps to first retrieve the meta data (column names) to update the resulting data.frame.
:::
...
::: {.alert .alert-block .alert-danger}
**Comprehension question 2.0.0:** Compare the structure information for the tibble version of our imported data in section **2.3.1** versus the above data frame version of the data. What differences do you notice about the columns? Name some differences between a tibble and a data frame.
:::
...
Using `unique()` we are returned a character vector containing 21 *C. elegans* strains.
...
brand <- c("wildrose", "guinness", "grasshopper")
wheatType <- c("Hard Red Spring", "Hard Red Winter", "Soft Red Winter" )
rating <- c(5, 7, ...)

# Put it all together with in a call to data.frame()
NA_example <- data.frame(brand, wheatType, rating)
...
mean(rating, ...) #Avoid using just "T" as an abbreviation for "TRUE"
```
...
What happened with our above command? Why did it return only 88 rows? To be honest, it was lucky that the operation worked at all! When R encounters operations between vectors of different size, it will recycle the shorter of the vectors when it can.
...
A powerful set of functions called regular expressions (regex) can also be used for partial matching. Regex is found in any programming language, not only in R, so familiarizing yourself with regex is a ***must*** as a programmer.
...
You will have until 12:59 hours on Wednesday, September 18th to submit your assignment (right before the next lecture).
